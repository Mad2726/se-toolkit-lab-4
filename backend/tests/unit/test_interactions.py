...
from app.routers.interactions import _filter_by_max_item_id
...
def test_filter_returns_all_when_max_item_id_is_none() -> None:
    ...
    result = _filter_by_max_item_id(interactions, None)
    assert result == interactions


def test_filter_returns_empty_for_empty_input() -> None:
    result = _filter_by_max_item_id([], 1)
    assert result == []


def test_filter_returns_interactions_below_max() -> None:
    interactions = [_make_log(1, 1, 1), _make_log(2, 2, 3)]
    result = _filter_by_max_item_id(interactions, 2)
    assert len(result) == 1
